...
export const rentalFileService = {
  ...
  // Obtenir la liste des documents manquants - FONCTION ORIGINALE RESTAURÉE
  getMissingDocuments(fileData: RentalFileData | null): string[] {
    ...
    const tenant = fileData.main_tenant
    const requiredDocs = this.getRequiredDocuments(tenant.situation)

    // Vérifier les documents d'identité
    if (requiredDocs.identity?.required && (!tenant.documents?.identity || tenant.documents.identity.length === 0)) {
      missingDocs.push("Pièce d'identité")
    }

    // Vérifier les justificatifs de revenus
    if (
      requiredDocs.income_proof?.required &&
      (!tenant.documents?.income_proof || tenant.documents.income_proof.length === 0)
    ) {
      missingDocs.push("Justificatifs de revenus")
    }

    // Vérifier l'avis d'imposition
    if (requiredDocs.tax_notice?.required && !tenant.documents?.tax_notice) {
      missingDocs.push("Avis d'imposition")
    }

    // Vérifier le justificatif de domicile
    if (requiredDocs.domicile_proof?.required && !tenant.documents?.domicile_proof) {
    ...
    switch (tenant.situation) {
      case "student":
        if (!tenant.documents?.other?.some((doc) => doc.includes("étudiant"))) {
          missingDocs.push("Carte d'étudiant ou certificat de scolarité")
        }
        break
      case "employee":
        if (!tenant.documents?.other?.some((doc) => doc.includes("contrat"))) {
          missingDocs.push("Contrat de travail")
        }
        break
      case "self_employed":
        if (!tenant.documents?.other?.some((doc) => doc.includes("kbis"))) {
          missingDocs.push("Extrait Kbis ou justificatif d'activité")
        }
        break
      case "unemployed":
        if (!tenant.documents?.other?.some((doc) => doc.includes("pôle emploi"))) {
          missingDocs.push("Attestation Pôle emploi")
        }
        break
      case "retired":
        if (!tenant.documents?.other?.some((doc) => doc.includes("pension"))) {
          missingDocs.push("Justificatif de pension")
        }
        break
    }
