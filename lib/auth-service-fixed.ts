...
// Fonction simplifiée basée sur les API qui fonctionnent
export async function getCurrentUserFromRequest(request: NextRequest): Promise<UserProfile | null> {
  try {
    ...
    // Récupérer le header Authorization si présent
    ...
    // Créer le client Supabase avec une approche simplifiée
    ...
    const supabase = createServerClient(supabaseUrl, supabaseKey, {
      cookies: {
        get(name: string) {
          ...
          console.log(`🍪 Cookie ${name}:`, cookie?.value ? "trouvé" : "non trouvé")
          return cookie?.value
        },
      },
    })
    ...
    // Utiliser le service client pour récupérer le profil
