          ...
          <CardContent className="flex items-center gap-3 py-6">
            ...
            <div>
              ...
              <Button variant="outline" size="sm" className="mt-2" asChild>
                <Link href="/tenant/rental-management/incidents">Retour aux incidents</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description={`Communication avec ${incident.lease.owner.first_name} ${incident.lease.owner.last_name}`}
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t">
              <h4 className="font-medium text-sm mb-2">Propriétaire</h4>
              <div className="flex items-center gap-2">
                <Avatar className="h-8 w-8">
                  <AvatarImage src="/placeholder.svg" />
                  <AvatarFallback>
                    {incident.lease.owner.first_name[0]}{incident.lease.owner.last_name[0]}
                  </AvatarFallback>
                </Avatar>
                <div>
                  ...
                  <p className="text-xs text-gray-600">{incident.lease.owner.email}</p>
                </div>
              </div>
            </div>
          </CardContent>
              ...
              <div className="space-y-4">
                {messages.length === 0 ? (
                  ...
                  messages.map((message) => (
                    <div
                      key={message.id}
                      className={`flex ${message.author_type === "tenant" ? "justify-end" : "justify-start"}`}
                    >
                      <div
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-medium">
                            {message.author_type === "tenant" ? "Vous" : message.author_name}
                          </span>
                        </div>
                        <p className="text-sm">{message.message}</p>
                        <p
                          className={`text-xs mt-1 ${
                            message.author_type === "tenant" ? "text-blue-100" : "text-gray-500"
                          }`}
                        >
