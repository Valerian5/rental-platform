...
export default function LeaseDetailPage() {
  ...
  const sendToTenant = async () => {
    try {
      ...
      // Afficher un message de succès (vous pouvez utiliser toast si disponible)
      alert("Bail envoyé au locataire avec succès !")
    } catch (error) {
  ...
  const signAsOwner = async () => {
    try {
      ...
      const response = await fetch(`/api/leases/${leaseId}/sign-owner`, {
        ...
        body: JSON.stringify({
          ownerId: "current-owner-id", // À remplacer par l'ID du propriétaire connecté
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {lease.status === "sent_to_tenant" && !lease.signed_by_owner && (
