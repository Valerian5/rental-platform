  ...
  const handleApplyModel = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/scoring-preferences/use-system", {
        ...
        body: JSON.stringify({
          owner_id: "current_user", // À remplacer par l'ID utilisateur réel
          system_preference_id: model.id,
        }),
      })

      if (!response.ok) throw new Error("Erreur lors de l'application du modèle")

      toast({
        title: "Modèle appliqué",
        description: `Le modèle "${model.name}" est maintenant votre configuration par défaut`,
      })
    } catch (error) {
