      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="analysis" className="space-y-4">
          ...
          <TabsContent value="documents" className="space-y-4">
            {rentalFile ? (
              <div className="space-y-6">
                {/* En-tête avec informations du dossier et bouton téléchargement */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <FileText className="h-5 w-5" />
                      Dossier de location - {rentalFile.main_tenant?.first_name} {rentalFile.main_tenant?.last_name}
                    </CardTitle>
                    <CardDescription>
                      Dossier complété à {rentalFile.completion_percentage || 0}% -
                      {1 + (rentalFile.cotenants?.length || 0) + (rentalFile.guarantors?.length || 0)} personne(s)
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-blue-600">
                            {rentalFile.completion_percentage || 0}%
                          </div>
                ...
                {/* Documents d'identité du locataire principal */}
                {hasDocuments(rentalFile.main_tenant?.identity_documents) && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Pièces d'identité du locataire</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                {hasDocuments(rentalFile.main_tenant?.activity_documents) && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Justificatifs d'activité</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                ...
                {hasDocuments(rentalFile.main_tenant?.income_sources?.work_income?.documents) && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Justificatifs de revenus</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                ...
                {rentalFile.cotenants &&
                  ...
                  rentalFile.cotenants.some((c: any) => hasDocuments(c.identity_documents)) && (
                    <Card>
                      <CardHeader>
                        <CardTitle>Documents des colocataires</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-6">
                        {rentalFile.cotenants.map(
                          (cotenant: any, cIndex: number) =>
                            hasDocuments(cotenant.identity_documents) && (
                              <div key={cIndex}>
                                ...
                                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                                  {cotenant.identity_documents.map((doc: string, index: number) => (
                                    <div key={index} className="border rounded-lg p-3">
                                      <div className="flex items-center gap-2 mb-2">
                ...
                {rentalFile.guarantors &&
                  ...
                  rentalFile.guarantors.some((g: any) => hasDocuments(g.personal_info?.identity_documents)) && (
                    <Card>
                      <CardHeader>
                        <CardTitle>Documents des garants</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-6">
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-10">
                  <FileText className="h-12 w-12 text-muted-foreground mb-4" />
                  <h3 className="text-lg font-medium text-gray-600">Dossier de location non disponible</h3>
