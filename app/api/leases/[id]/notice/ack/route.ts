...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const { data: notice } = await supabase
      .from("lease_notices")
      .select("notice_date, move_out_date")
      .eq("lease_id", leaseId)
    ...
    const { generateNoticeAckPdfBuffer } = await import("@/lib/notice-ack-generator")
    const pdfBuffer = generateNoticeAckPdfBuffer({
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ack-preavis-${leaseId}.pdf"`,
      },
    })
  } catch (error: any) {
    console.error("❌ notice/ack:", error)
