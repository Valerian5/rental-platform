    ...
    <Card className={`hover:shadow-lg transition-shadow ${isUpcoming() ? "border-blue-200 bg-blue-50/30" : ""}`}>
      <CardHeader className="pb-3">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
          <CardTitle className="text-base sm:text-lg font-semibold truncate">Visite #{visit.id.slice(0, 8)}</CardTitle>
          <Badge className={`${getStatusColor(visit.status)} text-xs whitespace-nowrap`}>
      ...
      <CardContent className="space-y-3">
        {visit.property && (
          <div className="p-3 bg-white rounded-md border">
            <h4 className="font-medium text-sm truncate">{visit.property.title}</h4>
            <div className="flex items-center text-xs text-gray-600 mt-1 min-w-0">
              <MapPin className="h-3 w-3 mr-1 flex-shrink-0" />
              <span className="truncate">{visit.property.address}</span>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <div className="flex items-center text-sm font-medium min-w-0">
            ...
            <span className="truncate">{formatDate(visit.visit_date)}</span>
          </div>
          ...
          {visit.tenant && (
            <>
              <div className="flex items-center text-sm min-w-0">
                ...
                <span className="font-medium truncate">{visit.tenant.name}</span>
              </div>
        ...
        {visit.notes && (
          <div className="p-2 bg-gray-50 rounded text-sm">
            <p className="line-clamp-2 break-words">{visit.notes}</p>
          </div>
        )}
