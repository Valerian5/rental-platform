...
export function VisitScheduler({
  ...
  const safeVisitSlots = getSafeSlots()
  ...
  const saveSlotsToDatabase = async (slots: VisitSlot[]) => {
    console.log("Saving slots to database:", slots)
  }
  ...
  const applyDayConfiguration = async () => {
    ...
    const newSlots: VisitSlot[] = dayConfig.selectedSlots.map((slotKey) => {
      ...
      return {
        ...
        max_visitors: dayConfig.capacity,
        current_bookings: 0,
        is_available: true,
  ...
  const clearDaySlots = async () => {
    ...
    if (mode === "management") {
      ...
      // CORRECTION: Recharger les créneaux depuis la DB après suppression
      await loadSlotsFromDatabase()
    } else {
