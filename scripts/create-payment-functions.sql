-- Fonctions pour le module de paiements
-- À exécuter dans Supabase SQL Editor

-- 1. Fonction pour générer les paiements mensuels
...
BEGIN
    ...
    due_date := DATE_TRUNC('month', (current_year || '-' || current_month || '-01')::DATE) + INTERVAL '1 month' - INTERVAL '1 day';
    ...
    SELECT 
        ...
        due_date,
        'pending' as status,
        'PAY-' || target_month || '-' || LPAD(EXTRACT(DAY FROM due_date)::TEXT, 2, '0') || '-' || SUBSTRING(l.id::TEXT, 1, 8) as reference
    FROM leases l
...
-- 2. Fonction pour marquer un paiement comme payé
...
-- 3. Fonction pour marquer un paiement comme impayé
...
BEGIN
    ...
    SET 
        ...
        payment_date = NULL,
        payment_method = NULL,
        notes = COALESCE(notes, 'Marqué comme impayé'),
        updated_at = NOW()
    WHERE id = payment_id;
...
-- 4. Fonction pour créer un rappel
...
-- 5. Fonction pour générer une quittance
...
BEGIN
    ...
    UPDATE payments 
    SET receipt_id = (SELECT id FROM receipts WHERE payment_id = payment_id ORDER BY created_at DESC LIMIT 1)
    WHERE id = payment_id;
...
-- 6. Fonction pour obtenir les statistiques d'un propriétaire
